...
The first element is the \verb|port| which identifies the address and the protocol to access the service (e.g. from the IBM online examples,\ref{IBMWSDL} \verb|<soap:address location="http://www.snowboard-info.co.uk/EndorsementSearch"/>|). With \verb|portType|, there are given the specifics of any \verb|operation| that can be used by the client, for example, the input/output parameters or the error messages. The \verb|operation|s comply to one of 4 main patterns: the \verb|One_way| (client sending a message to the provider), the \verb|Request_response| (client sending a message and waiting for an answer form the provider), the \verb|Solicit_response| (Provider sending a message to the client and waiting for a response) and the \verb|Notification| (provider sending a message to the client).
...
Eventually, we can see that WSDL proposes a static picture of the service. It defines the address and the way to access it, the operations and the input/output elements to be provided, but it has no notion of the order 
...
\textbf{WSDL element} 	& \textbf{Description}	\\ 
								\cmidrule(l){1-2}
port 		& It identifies the address where the service is found and the protocol to access it 				\\[0,1cm]
portType 	& It provides the description of the service, using the \textit{operation} element 				\\[0,1cm]
operation 	& It is a basic functionality provided by the service. It lists the input and output messages that are exchanged, in one of the four communication patterns 														\\[0,1cm]
message 	& It specifies the format of messages to be exchanged, e.g. the name and type of the variables they contain 	\\[0,1cm]
types		& The atomic or user-defined data types in use  								\\[0,1cm]
binding		& Given a portType, it describes how this is implemented in the specific protocol 				\\[0,1cm]
