...
The SOA presents an architecture devised in three main components as shown in Fig. \ref{The Service Oriented Architecture (SOA)}:
a \textit{service provider}, which makes the service available. The service provider register its service to the \textit{service registry}, which contains the list of available services and their descriptions. The \textit{service client} contacts the service registry to find a service, it gets a contract and the address of the service, making a so called \textit{binding}, between service provider and client.\\
